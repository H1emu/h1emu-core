...
mod tests {
    ...
    fn outoforder_parse_test_crc_fail() {
        ...
        let data_parsed: String = soeprotocol_class.parse(data_to_parse.to_vec());
        assert_eq!(data_parsed, r#"{"name":"Error","error":"CRC error"}"#)
    }
    ...
    fn ack_parse_test() {
        ...
        let data_parsed: String = soeprotocol_class.parse(data_to_parse.to_vec());
        assert_eq!(data_parsed, r#"{"name":"Ack","sequence":1}"#)
    }
    ...
    fn multi_parse_test() {
        ...
        assert_eq!(
            data_parsed,
            r#"{"name":"MultiPacket","sub_packets":[{"name":"Ack","sequence":206},{"name":"Data","sequence":1,"crc":0,"data":[0,25,41,141,45,189,85,241,64,165,71,228,114,81,54,5,184,205,104,0,125,184,210,74,0,247,152,225,169,102,204,158,233,202,228,34,202,238,136,31,3,121,222,106,11,247,177,138,145,21,221,187,36,170,37,171,6,32,11,180,97,10,246]}]}"#
        )
    }
    ...
    fn multi_pack_test() {
        let mut soeprotocol_class = Soeprotocol::initialize(false, 0);
        let data_to_pack:String = r#"{"sub_packets":[{"name":"Ack","sequence":206},{"name":"Data","sequence":1,"crc":0,"data":[0,25,41,141,45,189,85,241,64,165,71,228,114,81,54,5,184,205,104,0,125,184,210,74,0,247,152,225,169,102,204,158,233,202,228,34,202,238,136,31,3,121,222,106,11,247,177,138,145,21,221,187,36,170,37,171,6,32,11,180,97,10,246]}]}"#.to_owned();
    ...
    fn data_parse_test() {
        ...
        assert_eq!(
            data_parsed,
            r#"{"name":"Data","sequence":4,"crc":0,"data":[252,100,40,209,68,247,21,93,18,172,91,68,145,53,24,155,2,113,179,28,217,33,80,76,9,235,87,98,233,235,220,124,107,61,62,132,117,146,204,94,60]}"#
        )
    }
    ...
    fn data_parse_with_crc_test() {
        ...
        assert_eq!(
            data_parsed,
            r#"{"name":"Data","sequence":4,"crc":24124,"data":[252,100,40,209,68,247,21,93,18,172,91,68,145,53,24,155,2,113,179,28,217,33,80,76,9,235,87,98,233,235,220,124,107,61,62,132,117,146,204]}"#
        )
    }
