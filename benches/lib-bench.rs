...
fn soeprotocol_pack_benchmarks(c: &mut Criterion) {
    ...
    let multi_to_pack:String = r#"{"sub_packets":[{"name":"Ack","sequence":206},{"name":"Data","sequence":1,"data":[0,25,41,141,45,189,85,241,64,165,71,228,114,81,54,5,184,205,104,0,125,184,210,74,0,247,152,225,169,102,204,158,233,202,228,34,202,238,136,31,3,121,222,106,11,247,177,138,145,21,221,187,36,170,37,171,6,32,11,180,97,10,246]}]}"#.to_owned();
    let data_to_pack =
...
fn utils_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("is_pos_in_radius", |b| {
        b.iter(|| is_pos_in_radius(20.0,[0.0,1.0,2.0,0.0].to_vec(),[-19.0,1.0,20.0,0.0].to_vec()))
    });
}
