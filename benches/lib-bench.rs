...
fn crc_legacy_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("append_crc_legacy", |b| b.iter(|| append_crc_legacy(black_box(&data),black_box(0))));
    drop(data);
    let data:[u8;5] =  [0, 21, 0, 0, 2];
    c.bench_function("crc32_legacy", |b| b.iter(|| crc32_legacy(black_box(&data), black_box(0))));
    drop(data);
}
...
fn crc_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("append_crc", |b| b.iter(|| append_crc(black_box(&mut data),black_box(0))));
    drop(data);
    let mut data:Vec<u8> =  [0, 21, 0, 0, 2].to_vec();
    c.bench_function("crc32", |b| b.iter(|| crc32(black_box(&&mut data), black_box(0))));
    drop(data);
}
